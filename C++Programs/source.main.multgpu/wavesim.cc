...
double **wvenacd(double *vel, int nx, int ny,int srcloc, double freq,double h, double dt,double T){
	...
	start_idx[0] = 0; dom_start_idx[1] = topDomain_chunk;
	end_idx[0] = topDomain_chunk; dom_end_idx[1] = topDomain_chunk + botDomain_chunk;
	...
	{
	#pragma acc enter data copyin(U[0:5][start_idx[d]:end_idx[d]],Ux[0:5][start_idx[d]:end_idx[d]],Uy[0:5][start_idx[d]:end_idx[d]],stencil[0:nx*ny],vel[0:nx*ny],left_cfabc[0:ny][0:81],right_cfabc[0:ny][0:81],bottom_cfabc[0:nx][0:81],left_sstep[0:81],right_sstep[0:81],bottom_sstep[0:81],tstep[0:81])
	}
	...
	for (int i=0; i<nt-1;i++)
	{
		...
		#pragma acc parallel loop present(U[0:5][start_idx[0]:end_idx[0]],Ux[0:5][start_idx[0]:end_idx[0]],Uy[0:5][start_idx[0]:end_idx[0]])
		for (int j=0;j<Nx;j++){
			U[3][j]=U[3][j+2*Nx];
			Ux[3][j]=Ux[3][j+2*Nx];
			Uy[3][j]=Uy[3][j+2*Nx];
		...
		#pragma acc set device_num(0)
		#pragma acc kernels present(U[0:5][start_idx[0]:end_idx[0]])
		{
