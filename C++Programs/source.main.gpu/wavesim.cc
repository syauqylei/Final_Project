...
double **wvenacd(double *vel, int nx, int ny,int srcloc, double freq,double h, double dt,double T){
	...
	double t;
	#pragma acc data present(U[0:5][0:Nx*Ny],Ux[0:5][0:Nx*Ny],Uy[0:5][0:Nx*Ny])
	for (int i=0; i<nt-1;i++)
	{
		...
		#pragma acc parallel loop present(U[3][0:Nx],Ux[3][0:Nx],Uy[3][0:Nx],U[3][2*Nx:3*Nx],Ux[3][2*Nx:3*Nx],Uy[3][2*Nx:3*Nx])
		for (int j=0;j<Nx;j++){
			U[3][j]=U[3][j+2*Nx];
			Ux[3][j]=Ux[3][j+2*Nx];
			Uy[3][j]=Uy[3][j+2*Nx];
